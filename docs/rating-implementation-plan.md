...
| Категории турниров | ✅ | CRUD: `coefficient`, `ratingGte`, `ratingLt`, `segmentsCount` |
| Результаты соревнований | ✅ | `sum`, `diff`, `dnf`, `orderNumber` по каждому игроку |
...
5. В итоговый зачёт идут лучшие 4 лиги + лучшие 4 турнира + Чемпионат России
...
### Этап 4. Типизация категорий: лига / турнир / чемпионат

**Проблема:** Для правила «лучшие 4 лиги + лучшие 4 турнира + чемпионат» нужно знать **тип** соревнования. Сейчас в `tournament_categories` есть только `coefficient`, но нет поля, классифицирующего соревнование как лигу, турнир или чемпионат.

**Задачи:**

4.1. **Миграция БД** — добавить enum-поле в `tournament_categories`:
```sql
CREATE TYPE app_public.competition_class AS ENUM (
  'championship',  -- Чемпионат России (кат. 1)
  'tournament',    -- Федеральные / Межрегиональные (кат. 2, 3)
  'league'         -- Лиги (кат. 4, 5, 6)
);

ALTER TABLE app_public.tournament_categories
  ADD COLUMN competition_class app_public.competition_class NOT NULL DEFAULT 'league';
```
...
**Проблема:** Нет агрегации итогового рейтинга по правилу «лучшие 4 лиги + лучшие 4 турнира + чемпионат».

**Задачи:**

8.1. **Доменная функция** — `calculateSeasonTotal(playerStandings, season)`:
- Группировка по `competitionClass`: leagues, tournaments, championship
...
- Чемпионат считается полностью (1 событие)
...
- Таблица: место, игрок, дивизион, баллы (лиги + турниры + чемпионат = итого)
