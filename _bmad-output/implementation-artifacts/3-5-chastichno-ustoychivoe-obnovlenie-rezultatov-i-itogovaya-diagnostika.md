...
- [x] Соединить player/result flows из `3.2`, `3.3`, `3.4` в единый resilient orchestration path, который продолжает обработку при локальных ошибках. (AC: 1, 2)
- [x] Агрегировать skipped/problematic result units и причины пропуска в общий update summary/result contract. (AC: 2, 3)
- [x] Обеспечить согласованное поведение повторного запуска: корректные повторные records не создают дублей, проблемные записи не блокируют повторную обработку остальных. (AC: 1, 4)
- [x] Подготовить API-facing diagnostic payload для UI lifecycle/statistics story `1.5`, чтобы финальный результат был понятен пользователю. (AC: 3)
- [x] Добавить end-to-end-ish worker-level tests/fixtures для смешанного набора валидных, DNF и битых результатов. (AC: 1, 2, 3, 4)
...
- 2026-03-22: Added the final resilient results pipeline, structured diagnostics for players/results transport stages, and worker/API regression coverage for mixed and repeat-run scenarios.
...
- Reuse the existing results fetch boundary from `3.1`/`3.4`, but stop before result persistence so the final story can orchestrate `players` and `results` in one resilient flow.
- Extend the shared update contract with structured diagnostics sections while keeping the existing top-level summary shape stable for UI story `1.5`.
- Add a final worker job that persists players and competition results from the same fetched payload set, aggregates recoverable issues, and preserves idempotent repeat-run behavior.
- Cover the new pipeline with mixed-data and repeat-run tests, then verify worker/api/shared-types checks and tests.

### Debug Log References

- Added `apps/worker/src/jobs/results-pipeline-update-job.ts` to orchestrate shared fetch, player persistence, result persistence, and structured diagnostics in one resilient runtime path.
- Updated `apps/worker/src/orchestration/results-update.ts` so `/updates/results` now uses the final combined Epic 3 pipeline instead of the intermediate result-only job.
- Extended `packages/shared-types/src/updates/index.ts` with optional diagnostics sections for `transport`, `players`, and `results` while preserving the existing summary contract.
- Added `apps/worker/src/jobs/results-pipeline-update-job.test.ts` for mixed valid/DNF/broken payloads and repeat-run idempotency across both players and results.
- Updated `apps/api/src/app.test.ts` and `apps/worker/package.json` to validate the new diagnostics payload and include the final pipeline tests in the standard worker suite.
- Validation completed with `./node_modules/.bin/tsx --test apps/worker/src/jobs/results-pipeline-update-job.test.ts apps/worker/src/jobs/results-update-job.test.ts apps/worker/src/jobs/players-update-job.test.ts apps/api/src/app.test.ts`, `npm run check --workspace @metrix-parser/worker`, `npm run check --workspace @metrix-parser/api`, `npm run check --workspace @metrix-parser/shared-types`, `npm test --workspace @metrix-parser/worker`, and `npm test --workspace @metrix-parser/api`.

### Completion Notes List

- Added a final resilient `results` pipeline that fetches competition result payloads once and then persists valid players and competition results independently, so local failures do not stop the whole operation.
- Aggregated transport, player, and result issues into a single operation result while also exposing structured diagnostics sections that explain what was skipped and why.
- Kept the top-level summary compatible with the shared UI lifecycle contract from story `1.5`, while giving API consumers richer diagnostics for result updates.
- Preserved repeat-run safety by reusing the existing idempotent repositories for both `player_id` and `(competition_id, player_id, order_number)` matching.
- Added worker-level regression coverage for mixed valid, DNF, broken, and repeat-run scenarios, plus API coverage for the final diagnostics payload.

### File List

- packages/shared-types/src/updates/index.ts
- apps/worker/src/jobs/results-pipeline-update-job.ts
- apps/worker/src/jobs/results-pipeline-update-job.test.ts
