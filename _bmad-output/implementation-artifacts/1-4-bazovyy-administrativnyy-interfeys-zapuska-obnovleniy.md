...
- [x] Создать feature-модуль `apps/web/src/features/admin-updates/` с базовой страницей или экраном административного раздела вместо текущего placeholder в `App.tsx`. (AC: 1, 2, 3)
- [x] Добавить в админском интерфейсе четыре отдельные action-card/form блока: `competitions`, `courses`, `players`, `results`, чтобы пользователь видел различимые сценарии обновления из одного места. (AC: 1)
- [x] Для сценариев, где требуется период, добавить поля `dateFrom` / `dateTo` или эквивалентный парный ввод периода; для сценария парков оставить интерфейс без обязательного периода. (AC: 2)
- [x] Сделать запуск сценария только через явные кнопки/submit-действия пользователя, без auto-run при изменении формы, mount-эффектах или переходе на страницу. (AC: 3)
- [x] Добавить client-side API boundary в `apps/web/src/shared/api/` или feature-level API access слой для вызова backend endpoint'ов запуска обновлений; не использовать прямой доступ к Supabase или любым DB client'ам из `apps/web`. (AC: 4)
- [x] Подготовить минимальные shared/client types для команд обновления и ответа запуска, чтобы история `1.5` могла развивать lifecycle и result rendering на уже существующей contract-основе. (AC: 3, 4)
- [x] Если для доступа к админскому экрану нужен router-level entrypoint, аккуратно расширить `apps/web/src/app/router.tsx` и shell так, чтобы это не мешало будущей навигации Epic 4. (AC: 1)
- [x] Проверить статическую валидацию frontend части через `npm run check --workspace @metrix-parser/web` и smoke-проверить, что админский экран рендерится без обращения к БД напрямую. (AC: 1, 2, 3, 4)
...
- 2026-03-21: Added update trigger API contracts, admin update UI, and validation for Story 1.4; story moved to `review`.
- 2026-03-21: Applied code review fixes for accessibility and resilient API parsing; story moved to `done`.
...
- Added minimal `POST /updates/{operation}` routes plus CORS/preflight handling so the SPA can trigger backend commands from the Vite origin.
- Period validation was implemented only for `competitions`, `players`, and `results`, while `courses` intentionally remains period-free per PRD/epic requirements.
- `apps/api` test script was updated from `node --import tsx` to `node --loader tsx` because the repository currently runs on `Node v16.17.0`.
- Smoke verification for the frontend used `npm run check --workspace @metrix-parser/web`, `npm run build`, and an explicit repo scan confirming no `supabase` imports remain in `apps/web`.
- Final review follow-up removed the nested `<main>` landmark and made the shared web API client tolerant to empty or non-JSON responses.

### Completion Notes List

- Replaced the placeholder `App.tsx` content with a feature-first admin screen composed of four distinct update cards for competitions, courses, players, and results.
- Added a client-side API boundary in `apps/web/src/shared/api/` and wired every card to explicit submit-only `fetch` calls against backend update endpoints.
- Introduced shared update trigger request/response contracts in `packages/shared-types` so future stories can extend lifecycle and result rendering from a stable baseline.
- Added minimal backend update trigger endpoints in `apps/api/src/modules/updates` and covered them with API tests.
- Enabled minimal CORS handling in the API so the Vite SPA can call the backend during local development without direct database access.
- Added a lightweight route resolver in `apps/web/src/app/router.tsx` that keeps the admin screen compatible with future Epic 4 navigation work.
...
- Verified that `apps/web` does not import `supabase` or any direct DB client.
- Re-ran `npm run check --workspace @metrix-parser/web` and `npm run build` after the review fixes; both still passed.

### File List

- _bmad-output/implementation-artifacts/1-4-bazovyy-administrativnyy-interfeys-zapuska-obnovleniy.md
...
- apps/web/src/features/admin-updates/update-scenarios.ts
